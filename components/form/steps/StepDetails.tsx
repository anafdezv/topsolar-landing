    ...
    return (
        <>
            <span className='w-full flex flex-col gap-9 items-center'>
                ...
                <Input showLabel label="Full name" placeholder={'Enter your full name'} onChange={(e: string) => {
                    setData({ ...data, fullName: e })
                }} name={'fullName'} />
                <Input showLabel label="Phone number" placeholder={'Enter your phone number'} onChange={(e: string) => {
                    setData({ ...data, phoneNumber: e })
                }} name={'phoneNumber'} />
                <Input showLabel label="Email" placeholder={'Enter your email'} onChange={(e: string) => {
            ...
            <span className="flex flex-col md:flex-row w-full gap-2">
                ...
                <FormButtonNext disabled={!data.fullName || !data.email || !data.phoneNumber || !data.acceptPrivacy} onClick={onNext} />            </span>
        </>
    )
}
